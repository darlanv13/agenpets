...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<bool> _autenticarProfissionalFirebase(
    String cpfLimpo,
    String senha,
  ) async {
    // TRUQUE DO EMAIL FANTASMA
    final emailLogin = "$cpfLimpo@agenpets.pro";

    try {
      // 1. Tenta Login no Firebase Auth
      UserCredential userCred = await _auth.signInWithEmailAndPassword(
        email: emailLogin,
        password: senha,
      );

      // 2. Busca dados ATUALIZADOS do Firestore (para checar se está ativo e perfil)
      ...
      if (!doc.exists) {
        _mostrarSnack(
          "Erro de integridade: Perfil não encontrado.",
          cor: Colors.red,
        );
        await _auth.signOut();
        return false;
      ...
      if (data['ativo'] == false) {
        _mostrarSnack("Acesso revogado. Contate o gerente.", cor: Colors.red);
        await _auth.signOut();
        return false;
      }

      // 3. Sucesso! Define para onde vai
      _rotearProfissional(data);
      return true;
    } on FirebaseAuthException catch (e) {
      String msg = "Erro no login.";
      if (e.code == 'wrong-password') msg = "Senha incorreta.";
      if (e.code == 'user-not-found') msg = "Usuário não cadastrado no Auth.";
  ...
  // --- 3. ROTEAMENTO INTELIGENTE (MOBILE vs DESKTOP) ---
  void _rotearProfissional(Map<String, dynamic> proData) {
    ...
    bool isMaster =
        ...
        skills.contains('caixa');

    if (isMobile) {
      // No celular, sempre perguntamos se quer ir para o painel de cliente ou pro
