  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            const Icon(FontAwesomeIcons.fileMedical, size: 20, color: Color(0xFF4A148C)),
        ...
        actions: [
          TextButton.icon(
            ...
            label: const Text("FINALIZAR", style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            // 1. ANAMNESE
            _buildSectionHeader("1. Anamnese & Queixa Principal", FontAwesomeIcons.clipboardQuestion),
            const SizedBox(height: 10),
            _buildCard(
              child: Column(
                children: [
                  _buildTextField("Queixa Principal (Motivo da Consulta)", _queixaCtrl, maxLines: 2),
                  const SizedBox(height: 15),
                  _buildTextField("Histórico da Moléstia Atual (HMA)", _historicoCtrl, maxLines: 4),
                ],
              ),
            ),
            ...
            // 2. EXAME FÍSICO
            _buildSectionHeader("2. Exame Físico", FontAwesomeIcons.stethoscope),
            const SizedBox(height: 10),
            ...
            // 3. DIAGNÓSTICO & PLANO
            _buildSectionHeader("3. Avaliação Clínica", FontAwesomeIcons.userDoctor),
            const SizedBox(height: 10),
            _buildCard(
              child: Column(
                children: [
                  _buildTextField("Suspeita Clínica (Diferencial)", _suspeitaCtrl, maxLines: 2),
                  const SizedBox(height: 15),
                  _buildTextField("Diagnóstico Definitivo", _diagnosticoCtrl, maxLines: 2),
                  const SizedBox(height: 15),
                  _buildTextField("Plano Diagnóstico/Terapêutico", _planoCtrl, maxLines: 3),
                ],
              ),
            ),
            const SizedBox(height: 25),
            ...
            _buildSectionHeader("4. Conduta, Prescrição e Vacinas", FontAwesomeIcons.prescriptionBottleMedical),
            const SizedBox(height: 10),
            ...
            // 5. SERVIÇOS & COBRANÇA
            _buildSectionHeader("5. Serviços Realizados (Cobrança)", FontAwesomeIcons.fileInvoiceDollar),
            const SizedBox(height: 10),
            _buildBillingCard(),
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text("Itens a Cobrar", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[700])),
              TextButton.icon(
                ...
                onPressed: () async {
                  final result = await showDialog<List<Map<String, dynamic>>>(
                    context: context,
                    builder: (ctx) => ServicosSelectDialog(initialSelected: _itensCobranca),
                  );
                  if (result != null) {
          ...
          if (_itensCobranca.isEmpty)
            const Padding(
              ...
              child: Center(child: Text("Nenhum serviço lançado.", style: TextStyle(color: Colors.grey))),
            )
          else
            ListView.builder(
              shrinkWrap: true,
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Text("R\$ ${item['preco']}", style: const TextStyle(fontWeight: FontWeight.bold)),
  ...
  Future<void> _finalizarConsulta() async {
    if (_queixaCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Preencha a Queixa Principal."), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      ...
      await _service.salvarConsultaCompleta(
        ...
        anamnese: anamnese,
        fisico: {
          'sinais_vitais': _vitalsData,
          'sistemas': _systemsData,
        },
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Prontuário salvo com sucesso!"), backgroundColor: Colors.green),
        );
