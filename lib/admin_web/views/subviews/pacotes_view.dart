...
class _PacotesViewState extends State<PacotesView> {
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isAtivo ? Colors.green.withValues(alpha: 0.1) : Colors.grey[100],
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: ListView(
                      children: _buildResumoItens(data),
                    ),
                  ),
                ],
              ),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    ...
                    onPressed: () => _abrirEditorPacote(context, docId: doc.id, data: data),
                    child: Text("Editar"),
                  ),
                ),
                SizedBox(width: 10),
  ...
  List<Widget> _buildResumoItens(Map<String, dynamic> data) {
    ...
    if ((data['vouchers_banho'] ?? 0) > 0) {
      widgets.add(_itemRow("Banho", data['vouchers_banho'], FontAwesomeIcons.shower, Colors.blue));
    }
    if ((data['vouchers_tosa'] ?? 0) > 0) {
      widgets.add(_itemRow("Tosa", data['vouchers_tosa'], FontAwesomeIcons.scissors, Colors.orange));
    }
    if (data['itens_extra'] != null) {
      for (var item in data['itens_extra']) {
        widgets.add(_itemRow(item['servico'], item['qtd'], FontAwesomeIcons.plus, Colors.purple));
      }
    }
    return widgets;
  }
  ...
  void _abrirEditorPacote(BuildContext context, {String? docId, Map<String, dynamic>? data}) {
    // Controllers e State
    ...
    if (data != null) {
      if ((data['vouchers_banho'] ?? 0) > 0) _itens.add({'servico': 'Banho', 'qtd': data['vouchers_banho']});
      if ((data['vouchers_tosa'] ?? 0) > 0) _itens.add({'servico': 'Tosa', 'qtd': data['vouchers_tosa']});
      if (data['itens_extra'] != null) {
        for (var item in data['itens_extra']) {
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: _corAcai),
                        ),
                        ...
                        Expanded(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Detalhes do Plano", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                SizedBox(height: 20),
                                TextField(
                                  controller: _nomeCtrl,
                                  decoration: InputDecoration(labelText: "Nome do Pacote", filled: true, fillColor: Colors.grey[50]),
                                ),
                                SizedBox(height: 15),
                                Row(
                                  children: [
                                    Expanded(
                                      child: TextField(
                                        ...
                                        decoration: InputDecoration(labelText: "Preço (R\$)", filled: true, fillColor: Colors.grey[50]),
                                      ),
                                    ),
                                    SizedBox(width: 15),
                                    Expanded(
                                      child: DropdownButtonFormField<String>(
                                        ...
                                        decoration: InputDecoration(labelText: "Porte", filled: true, fillColor: Colors.grey[50]),
                                        items: ['Pequeno Porte', 'Médio Porte', 'Grande Porte'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(),
                                ...
                                TextField(
                                  ...
                                  maxLines: 3,
                                  decoration: InputDecoration(labelText: "Descrição App", filled: true, fillColor: Colors.grey[50]),
                                ),
                                Spacer(),
                                SwitchListTile(
                        ...
                        Expanded(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Composição", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                SizedBox(height: 15),
                                Wrap(
                                  spacing: 10,
                                  children: [
                                    ActionChip(
                                      ...
                                      avatar: Icon(FontAwesomeIcons.shower, size: 14),
                                      onPressed: () => _addItem("Banho"),
                                    ),
                                    ActionChip(
                                      label: Text("Add Tosa"),
                                      avatar: Icon(FontAwesomeIcons.scissors, size: 14),
                                    ...
                                    StreamBuilder<QuerySnapshot>(
                                      ...
                                      builder: (context, snap) {
                                        ...
                                        return PopupMenuButton<String>(
                                          ...
                                          itemBuilder: (ctx) => snap.data!.docs.map((d) {
                                            String nome = d['nome'];
                                  ...
                                  child: Container(
                                    ...
                                    child: ListView.separated(
                                      ...
                                      itemBuilder: (ctx, i) {
                                        final item = _itens[i];
                                        return ListTile(
                                          title: Text(item['servico'], style: TextStyle(fontWeight: FontWeight.bold)),
                                          trailing: Row(
                                            mainAxisSize: MainAxisSize.min,
                                            children: [
                                              IconButton(icon: Icon(Icons.remove_circle, color: Colors.grey), onPressed: () => _removeItem(i)),
                                              Text("${item['qtd']}", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                                              IconButton(icon: Icon(Icons.add_circle, color: _corAcai), onPressed: () => _addItem(item['servico'])),
                                            ],
                                          ),
                                        );
                                      },
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 15),
                        ElevatedButton(
                          style: ElevatedButton.styleFrom(backgroundColor: _corAcai, padding: EdgeInsets.symmetric(horizontal: 30, vertical: 15)),
                          onPressed: () => _salvarPacote(context, docId, _nomeCtrl.text, _precoCtrl.text, _porteSelecionado, _descCtrl.text, _ativo, _itens),
                          child: Text("SALVAR PACOTE", style: TextStyle(color: Colors.white)),
                        ),
                      ],
                    ),
                  ),
  ...
  void _salvarPacote(BuildContext context, String? docId, String nome, String precoStr, String porte, String desc, bool ativo, List<Map<String, dynamic>> itens) async {
    ...
    for (var item in itens) {
      ...
      int qtd = item['qtd'];
      if (servico == 'Banho') totalBanho += qtd;
      else if (servico == 'Tosa') totalTosa += qtd;
      else {
        extrasVisual.add(item);
        // Gera chave segura para vouchers dinâmicos
        String key = "vouchers_" + servico.toLowerCase().replaceAll(RegExp(r'[^a-z0-9]'), '_');
        vouchersDinamicos[key] = (vouchersDinamicos[key] ?? 0) + qtd;
      }
    }

    Map<String, dynamic> data = {
      'nome': nome, 'preco': preco, 'porte': porte, 'descricao': desc,
      'ativo': ativo, 'visivel_app': ativo,
      'vouchers_banho': totalBanho, 'vouchers_tosa': totalTosa,
