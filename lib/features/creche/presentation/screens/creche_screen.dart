...
import 'package:agenpet/features/creche/data/creche_repository.dart';
import 'package:agenpet/features/auth/data/auth_repository.dart';
import 'package:agenpet/features/store/data/store_repository.dart';
import 'package:agenpet/features/subscription/data/subscription_repository.dart';
...
class _CrecheScreenState extends State<CrecheScreen> {
  ...
  final _crecheRepository = CrecheRepository();
  ...
  Future<void> _carregarEnderecoSalvo() async {
    ...
    final end = await _authRepository.getUserAddress(_userCpf!);
    if (end != null && mounted) {
      setState(() {
  ...
  Future<void> _carregarDisponibilidade() async {
    final datas = await _crecheRepository.buscarDiasLotadosCreche();
    if (mounted) {
      setState(() {
        _diasLotados = datas
  ...
  Future<void> _carregarConfigs() async {
    try {
      final config = await _storeRepository.getConfiguracoes();
      if (mounted) {
        setState(() {
  ...
  Future<void> _fazerReserva() async {
    ...
    try {
      await _crecheRepository.reservarCreche(
        petId: _petId!,
        cpfUser: _userCpf!,
      ...
      if (_usaTaxiDog) {
        _authRepository.saveUserAddress(
          _userCpf!,
  ...
  Widget _buildStepResumo() {
    return StreamBuilder<Map<String, int>>(
      stream: _userCpf != null
          ? _subscriptionRepository.getSaldoVouchers(_userCpf!)
